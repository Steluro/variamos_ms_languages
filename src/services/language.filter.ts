...
const builders: ((f: LanguageFilter) => Partial<WhereOptions> | undefined)[] = [
  (f) =>
    f.uuid
      ? {
          uuid: {
            [Op.in]: Array.isArray(f.uuid)
              ? f.uuid
              : f.uuid.split(",").map((id) => id.trim()),
          },
        }
      : undefined,
  ...
  (f) =>
    f.type
      ? {
          type: {
            [Op.in]: Array.isArray(f.type)
              ? f.type
              : f.type.split(",").map((type) => type.trim()),
          },
        }
      : undefined,
  ...
  (f) =>
    f.status
      ? {
          status: {
            [Op.in]: Array.isArray(f.status)
              ? f.status
              : f.status.split(",").map((status) => status.trim()),
          },
        }
      : undefined,
  ...
  (f) =>
    f.updatedBefore || f.updatedAfter
      ? {
          updatedAt: {
  ...
  (f) =>
    f.ownerId
      ? {
          ownerId: {
            [Op.in]: Array.isArray(f.ownerId)
              ? f.ownerId
              : f.ownerId.split(",").map((id) => id.trim()),
          },
        }
      : undefined,
  ...
  (f) =>
    f.ownerName
      ? {
          [Op.or]: (Array.isArray(f.ownerName)
            ? f.ownerName
            : f.ownerName.split(",")
          ).map((name) => ({
